/**
 * Gateway-side webhook dispatch (enqueue via Redis for worker pickup).
 * Uses fetch directly for simulate-bounce in dev; worker handles normal delivery webhooks.
 */
import { createHmac, randomUUID } from 'node:crypto';
import { loadConfig, type WebhookEvent } from '@vmds/shared';

export async function dispatchWebhookFromGateway(
