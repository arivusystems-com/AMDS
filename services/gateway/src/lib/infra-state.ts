...
export async function getSmtpFailureRate(): Promise<number> {
...
export async function getSimulatedQueueDepth(): Promise<number | null> {
  const redis = getRedis();
  const value = await redis.get(SIM_QUEUE_KEY);
  return value !== null ? Number.parseInt(value, 10) : null;
}

export async function setSimulatedInfraPressure(input: {
  queue_depth?: number;
  smtp_failure_rate?: number;
}): Promise<void> {
  const redis = getRedis();
  if (input.queue_depth !== undefined) {
    await redis.set(SIM_QUEUE_KEY, String(input.queue_depth), 'EX', 3600);
  }
  if (input.smtp_failure_rate !== undefined) {
    await redis.set(SIM_SMTP_KEY, String(input.smtp_failure_rate), 'EX', 3600);
  }
}

export async function clearSimulatedInfraPressure(): Promise<void> {
  const redis = getRedis();
  await redis.del(SIM_QUEUE_KEY, SIM_SMTP_KEY);
}

export async function resolveInfraMultiplier(queueDepth: number): Promise<number> {
  const config = loadConfig();
...
export async function getInfraPressureSnapshot(queueDepth: number) {
  const config = loadConfig();
  const simulatedDepth = await getSimulatedQueueDepth();
  const smtpRate = await getSmtpFailureRate();
  const effectiveDepth = simulatedDepth ?? queueDepth;

  return {
    queue_depth: effectiveDepth,
    queue_depth_threshold: config.INFRA_QUEUE_DEPTH_THRESHOLD,
    smtp_failure_rate: smtpRate,
    smtp_failure_threshold: config.INFRA_SMTP_FAILURE_THRESHOLD,
    simulated: simulatedDepth !== null,
    multiplier: await resolveInfraMultiplier(queueDepth),
  };
}
